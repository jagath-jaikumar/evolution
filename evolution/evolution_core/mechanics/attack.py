from evolution.evolution_core.cards.traits import (
    Nocturnal, Swimming, HighBodyWeight, Camouflage, Burrowing, Transparent, 
    Partnership, Flight, Patronage, DetrimentalMutation, Horned, Running, 
    Repelling, TailLoss, InkCloud, Anglerfish, Poisonous, SharpVision, TraitClass, Trait)
...
HARD_PROT_TRAITS = list([
    ...
    Transparent(), Partnership(), Flight(), Patronage()
])
...
def attack_result(att_player: Player, predator: Animal, def_player: Player, prey: Animal):
    if any(isinstance(trait, Swimming) for trait in predator.traits) and not any(isinstance(trait, Swimming) for trait in prey.traits):
        return 0
